...
function DragNDrop({ newData, changeRender }) {
  ...
  const setWorkOwner = async (work_id, work_owner, work_status) => {
    try {
      ...
      await axios.put("http://localhost:3000/dashboard/setWorkOwner", body);
  ...
  const handleDeleteWork = async (work_id) => {
    await axios
      .delete(`http://localhost:3000/dashboard/delete/:${work_id}`)
      .then((response) => {
        ...
        >
          ...
          {group.items.map((item, itemIndex) => (
            <div
              ...
              key={itemIndex}
              draggable={
                item.work_owner === 0 || userId === item.work_owner
                  ? true
                  : false
              }
              onDragStart={(e) => {
            ...
            >
              ...
              >
                ...
                <p className="card-paragraph">
                  {" "}
                  Projeyi Oluşturan Kişi: {item.name} {item.surname}
                </p>
                <p className="card-paragraph">
                  {" "}
                  Proje Önceliği: {item.priority}
                </p>
                <p className="card-paragraph">
                  {" "}
                  Projeyi Alan Kişi:{" "}
                  {item.work_owner ? `${item.work_owner}` : "Bilinmiyor"}
                </p>
              </div>
            </div>
          ))}
